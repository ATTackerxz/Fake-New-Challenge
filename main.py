...
if __name__ == '__main__':
    ...
    # 划分数据集，按照9:1划分
    train_len = int(len(train_all_dataset) * 8 / 10)
    ...
    for i in range(epoch):
        ...
        for index, (text, label) in enumerate(train_data_loader, start=1):
            optimizer.zero_grad()
            output = model(text)
            loss = F.nll_loss(output.cuda(), label.cuda()).cuda()
        ...
        for index, (text, label) in enumerate(validation_data_loader, start=1):
            with torch.no_grad():
                output = model(text).cuda()
                loss = F.nll_loss(output.cuda(), label.cuda())
                total_loss += loss.item()
                predict = output.argmax(dim=1)
                correct += predict.eq(label.cuda()).sum().item()
